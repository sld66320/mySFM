...
void compVec(std::vector<double> kp, double k, std::vector<double> p, std::vector<double> &result)
{
    if(kp.size() != 4 || p.size() != 4)
        return;
    for(int i = 0; i < 4; i++)
    {
        result[i] = kp[i]*k - p[i];
